...
_FONT_SIZE = 1
_FONT_THICKNESS = 1
_FPS_AVERAGE_FRAME_COUNT = 20

# Set the DISPLAY environment variable to use the dummy backend
...
def run(model: str, max_results: int, score_threshold: float, num_threads: int,
  ...
  while cap.isOpened():
    ...
    # Show the FPS
    fps_text = 'Calculated FPS = ' + str(int(fps))
    text_location = (_LEFT_MARGIN, _ROW_SIZE)
    cv2.putText(image, fps_text, text_location, cv2.FONT_HERSHEY_PLAIN,
                _FONT_SIZE, _TEXT_COLOR, _FONT_THICKNESS)

    # Show the original video fps
    video_fps_text = 'Original FPS = ' + str(int(video_fps))
    fps_text_location = (_LEFT_MARGIN, 2 * _ROW_SIZE)
    cv2.putText(image, video_fps_text, fps_text_location, cv2.FONT_HERSHEY_PLAIN,
                _FONT_SIZE, _TEXT_COLOR, _FONT_THICKNESS)
